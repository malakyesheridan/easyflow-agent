...
import { acknowledgeSignalEvent } from '@/lib/mutations/signal_events';
...
/**
 * POST /api/operations/intelligence/:signalEventId/acknowledge
 * Body: { orgId?, acknowledgedByUserId? }
 */
export async function POST(req: Request, { params }: RouteParams): Promise<Response> {
  const handler = withRoute(async (request: Request) => {
    ...
    return await acknowledgeSignalEvent({
      orgId: context.data.orgId,
      signalEventId,
      acknowledgedByUserId: body?.acknowledgedByUserId ?? context.data.actor.userId ?? null,
    });
  });
